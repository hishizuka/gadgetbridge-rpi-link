...
- **在 Raspberry Pi 显示屏上显示 Android 通知。** Gadgetbridge 会转发 Android
  通知的新增、更新和删除数据包，本库将其解析为带类型的通知事件，供宿主 UI 显示。
...
重要限制：HTTP 适用于文本和小型负载，通常需要 Bangle.js 版本的 Gadgetbridge；
...
`gadgetbridge_rpi_link`。解析和会话模块仅使用 Python 标准库，发行包会安装
`bluez-peripheral`以支持 BlueZ 主机功能。
...
发送消息时可以传入字典。`FrameEncoder`会添加 Gadgetbridge UART TX 通道所需的
帧格式和结尾换行符。
...
从 UART 数据流收到带`GB(...)`包装的 Gadgetbridge 消息后即可进行解析：
...
使用`t`命令时，请输入一行不含接收端`GB(...)`包装的 Gadgetbridge 消息。请直接
使用 URL，而不要使用 Markdown 链接。类 JSON 对象语法和严格 JSON 均可使用。例如：
...
仅当宿主应用确实希望在收到`is_gps_active`后请求开启 Gadgetbridge GPS 时，才应使用
`--auto-gps`。

仓库中还保留了`examples/callback_bluez_service.py`，它是同一安装命令的精简源码
检出包装器。
...
| Google Maps 导航 | 允许其通知。如果收到的`nav`数据包为空，请参阅[导航通知](#导航通知)。 |
...
- 对于 HTTP 请求（`t:"http"`），最简单的方法是使用可直接访问互联网的 Bangle.js
  版本。可通过以下任一方式获取：
...
Gadgetbridge 的 Bangle.js UART 实现目前在
`app/src/main/java/nodomain/freeyourgadget/gadgetbridge/service/devices/banglejs/BangleJSDeviceSupport.java`
中为 UART 字符串路径使用`StandardCharsets.ISO_8859_1`。在已检查的源代码中，
手机到设备的发送路径（`uartTx(...)`）和设备到手机的接收路径都使用了该编码。如需
通过此 UART 协议直接交换多字节文本，需要使用自定义 Gadgetbridge 版本，将这些
Bangle.js UART 字符集设置更改为`StandardCharsets.UTF_8`。在 Android 开发者模式下
安装该自定义版本后，本库即可接收 UTF-8 多字节消息。源代码构建方法请参阅
...
Google Maps 导航依赖通知。Gadgetbridge 从 Android 通知中读取逐向导航指示，并以
`nav`数据包发送。在较新的 Android 版本中，Google Maps 的实时通知类别可能使
Gadgetbridge 无法读取指示详情。如果收到的`nav`数据包不包含`action`、`distance`
等字段，请在 Android 的 Google Maps 通知设置中禁用`Live Updates`、`Live info`或
名称类似的通知类别。

### Android Intent

发送`t:"intent"`数据包需要在已注册设备的设置中启用`Allow Intents`
（`device_intents`）。在 Gadgetbridge 源代码中，关闭此开关时`handleIntent(...)`
会拒绝 Intent 数据包。Gadgetbridge 的说明中还指出，在后台使用此功能可能需要
Android 的“显示在其他应用上层”权限。
...
`Allow Internet Access`（`device_internet_access`）。

## HTTP 下载行为

`GadgetbridgeSession.download_http_file(...)`会写出 Gadgetbridge HTTP 响应中的
`resp`字段。当前 Android 版 Gadgetbridge 的 Bangle.js HTTP 支持面向文本，因此应将
此辅助函数视为文本和小型负载的下载通道。

`binary=None|True|False`选项仅控制如何将已经收到的负载写入磁盘：

- `None`：仅当输出路径具有`.bin`、`.fit`、`.zip`等已知二进制扩展名时，才将字符串
  负载写为二进制。
- `True`：尽可能将旧式 Gadgetbridge 二进制字符串保留为 Latin-1 字节；对于从
  `atob(...)`解码的 Unicode 文本，则回退到 UTF-8。
- `False`：即使输出路径扩展名通常表示二进制文件，也会使用指定编码将字符串负载
  写为文本。

字节负载始终按字节写入。本库不会嗅探响应内容；宿主应明确指定预期的下载路径，或在
写入约定更清晰时传入`binary=True|False`。

当前 Gadgetbridge Android 端的`t:"http"`实现不支持任意二进制 HTTP 下载。通过
Android 文本通道返回的大型二进制响应可能在 Gadgetbridge 端已经被插入替换字符，
宿主可能无法恢复。要支持通用二进制下载，需要将 Gadgetbridge 端 HTTP 处理程序改为
保留字节的响应通道，例如使用原始字节传输，或使用文档化的 base64/`atob(...)`约定，
而不是文本读取器。

## 开发说明

源码树不需要硬编码的 BLE 地址、设备主机名或私有本地文件系统路径。测试使用合成
负载、`example.com`以及公开的 Espruino 文本示例 URL。`__pycache__`等生成文件、
